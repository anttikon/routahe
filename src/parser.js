...
function parseTime(time) {
  if (time && time.startsWith('@')) {
    return moment(time.slice(1), ['HH:mm', 'H:mm'], true)
  } else {
    return moment(time, ['HH:mm', 'H:mm'], true)
  }
}

export const parseDateTime = (opts) => {
  const parsedDate = moment(opts.date, ['DD.MM.YYYY', 'DD.MM.', 'DD.MM', 'DD.M.YYYY', 'DD.M.', 'DD.M', 'D.M', 'D.MM', 'D.MM.', 'D.M.YYYY', 'D.MM.YYYY'], true)
